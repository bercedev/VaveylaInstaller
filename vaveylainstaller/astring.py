...
import sys
import subprocess
from vaveylainstaller import hata, bilgi, onemli, soru
...
def main():
    ...
    if Sonuc == "2":
        ...
        if soup.title.string == "Create new application":
            ...
            app_title = soru("Uygulamanızın adı ne olsun? (Otomatik oluşturmak için enter tuşuna basın): ")
            if app_title == '':
                app_title = choice(["va", "vav", "vave", "vavey", "telethon", "pyrogram"]) + choice(["", "-", "+"]) + choice(["user", "yla", "vue", "jsx", "python", "eyla"]) + choice([str(randint(1000, 99999)), ""])
            
            app_shortname = soru("APP'ın kısa adı ne olsun? (Otomatik Oluşturmak için enter tuşuna basın) \[5-32 karakter\]: ")
            if app_shortname == '':
                app_shortname = choice(["va", "vav", "vave", "madelineproto", "telethon", "pyrogram"]) + choice(["", "-", "+"]) + choice(["user", "yla", "vue", "jsx", "python", "php"]) + choice([str(randint(10000, 99999)), ""])
            ...
            AppInfo = {
                ...
                "app_desc": choice(["madelineproto", "pyrogram", "vaveyla", "telethon", "", "web", "cli"])
            }
