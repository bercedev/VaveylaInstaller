...
def createApp (connect):
    appname = "vaveyla" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
        ...
        exit(1)
    return appname
RanPassword = "eylaU" # Auto deployun çalışması için değiştirmeyin!
...
async def botlog (String, Api, Hash):
    ...
    KanalId = await Client(CreateChannelRequest(
        title='VaveylaUserBot BotLog',
        about=LANG['AUTO_BOTLOG'],
        megagroup=True
    ))
    KanalId = KanalId.chats[0].id
    
    Photo = await Client.upload_file(file='vaveylalogo.jpg')
...
if __name__ == "__main__":
    ...
    SyperStringKey = DefAppKey + "Bot"
    GiperStringKey = must + "4f/"
    ...
    config['DEFAULT_BIO'] = "En iyinin en iyisi olma yolunda.. ✨ ༺ @VaveylaUserBot "
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/must4f/VaveylaUserBot"
    config['WARN_LIMIT'] = "3"
